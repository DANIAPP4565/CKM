...
APP_TITLE = "CKM + CRHM | Detección, estadificación y manejo integrado"
...
.main-title {
  font-size: 2.1rem; line-height: 1.1; font-weight: 850; letter-spacing: -0.03em;
  color: var(--text); margin-bottom: 0.35rem;
}
.subtitle {color: var(--muted); font-size: 1.02rem; margin-bottom: 1rem;}
.card {
...
def domain_chart(scores: Dict[str, int]) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Bar(x=list(scores.keys()), y=list(scores.values()), text=list(scores.values()), textposition="auto"))
    fig.update_layout(
        height=330,
        margin=dict(l=20, r=20, t=30, b=20),
        yaxis=dict(range=[0, 3], tickvals=[0, 1, 2, 3], title="Carga de dominio"),
        xaxis=dict(title=""),
        title="Mapa de carga por dominios CKM/CRHM",
        showlegend=False,
    )
    return fig


def stage_gauge(stage_n: int) -> go.Figure:
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "bar": {"thickness": 0.25},
            "steps": [
                {"range": [0, 1], "color": "#ecfdf5"},
                {"range": [1, 2], "color": "#fefce8"},
                {"range": [2, 3], "color": "#fffbeb"},
                {"range": [3, 4], "color": "#fef2f2"},
            ],
        },
        title={"text": "Etapa CKM"},
    ))
...
TEMPLATE_COLUMNS = [
    ...
    "masld_known", "liver_stage", "ascvd", "stroke_tia", "pad", "hf", "af", "cac_gt100", "pre_hf", "prevent_10", "prevent_30",
]


def template_csv_bytes() -> bytes:
    df = pd.DataFrame([{col: "" for col in TEMPLATE_COLUMNS}])
    df.loc[0, "patient_code"] = "CKM-001"
    df.loc[0, "age"] = 58
    df.loc[0, "sex"] = "Hombre"
    df.loc[0, "weight_kg"] = 92
    df.loc[0, "height_cm"] = 172
    df.loc[0, "waist_cm"] = 108
    df.loc[0, "sbp"] = 138
    df.loc[0, "dbp"] = 86
    df.loc[0, "on_bp_meds"] = True
    df.loc[0, "fpg"] = 112
    df.loc[0, "hba1c"] = 6.1
    df.loc[0, "tg"] = 190
    df.loc[0, "hdl"] = 38
    ...
    df.loc[0, "ast"] = 34
    df.loc[0, "alt"] = 42
    df.loc[0, "platelets"] = 240
    df.loc[0, "masld_known"] = True
    df.loc[0, "liver_stage"] = "Desconocido"
    df.loc[0, "prevent_10"] = 12
    df.loc[0, "prevent_30"] = 35
...
def row_to_patient(row: pd.Series) -> Dict[str, Any]:
    ...
    for bcol in ["on_bp_meds", "known_t2d", "masld_known", "ascvd", "stroke_tia", "pad", "hf", "af", "cac_gt100", "pre_hf"]:
    ...
    p["subclinical_atherosclerosis"] = False
    p["osa_suspected"] = False
    return p
...
def main() -> None:
    ...
    st.markdown(
        f"<div class='subtitle'>Herramienta clínica operativa para estadificación CKM, extensión CRHM, FIB-4, KDIGO, síndrome metabólico y propuesta terapéutica integrada. <b>{AUTHOR}</b> · {VERSION}</div>",
    ...
    with tab1:
        ...
        with cc1:
            st.plotly_chart(stage_gauge(r["ckm_stage_n"]), use_container_width=True)
        with cc2:
            st.plotly_chart(domain_chart(r["domain_scores"]), use_container_width=True)
