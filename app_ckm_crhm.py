...
# IMPORTANTE:
# Esta versión NO usa Plotly.
# Motivo: en Streamlit Cloud algunos despliegues fallan si la dependencia no se instala
# antes de ejecutar la app. Se usan componentes nativos de Streamlit para evitar
# ModuleNotFoundError y mejorar la robustez del deploy.
...
def domain_chart(scores: Dict[str, int]) -> pd.DataFrame:
    """Devuelve un DataFrame simple para graficar con st.bar_chart sin dependencias externas."""
    return pd.DataFrame({
...
def main() -> None:
    ...
    with tab1:
        ...
        with cc1:
            ...
            st.progress(stage_progress_value(r["ckm_stage_n"]))
            st.caption(f"Etapa {r['ckm_stage_n']} de 4")
        with cc2:
            st.markdown("#### Mapa de carga por dominios CKM/CRHM")
            st.bar_chart(domain_chart(r["domain_scores"]))
